...
def setup_logger(
    ...
    Args:
        name:         Logger name (use __name__ in modules, or a fixed
                      name like "scraper" for the app-wide logger built
                      once in Container).
        level:        Log level string (DEBUG / INFO / WARNING / ERROR).
        log_format:   'json' for structured/production logs, 'text' for
                      local dev readability.
        log_file:     Path to log file; None/empty = console only.
        extra_fields: Arbitrary fields embedded in every JSON record
                      (e.g. {"base_url": settings.base_url}). Ignored
    ...
    if log.handlers:
        return log  # already configured — avoids duplicate handlers
                    # if setup_logger() is ever called twice for the same name
...
# Back-compat alias — in case anything in the codebase still imports
# build_logger by its old name. Prefer setup_logger() in new code.
def build_logger(name: str, level: str = "INFO") -> logging.Logger:
    return setup_logger(name=name, level=level, log_format="text", log_file=None)